...
# create dictionaries showing properties of transcript that meets criteria HNF4A, canonical, exon 1-9, <c.1258
# two dictionaries, first for transcripts meeting PVS1, second for those meeting PVS1_supporting
        ...
        #this dict is for transcripts in exon 10 3' of c.1258
        can_hnf4a_dict2 = [d2 for d2 in decoded_1
                            if d2.get('gene_symbol') == 'HNF4A'
                            and d2.get('canonical') == 1
                            and int(d2.get('cds_end', 0)) == range(1258, 3180)]

        # message to be printed for variants not meeting PVS1
        if not can_hnf4a_dict1 or can_hnf4a_dict2:
                print('PVS1 not met')

        # variants that satisfy HNF4A, canonical, exon1-9 - need to be filtered by variant type.
        ...
        #same for those that are in exon 10
...
#Had added this but I think it is unnecessary
#def __repr__(self):
